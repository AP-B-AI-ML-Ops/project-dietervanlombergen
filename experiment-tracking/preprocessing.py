...
def load_raw_data(data_dir: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    sun_df = pd.read_csv(os.path.join(data_dir, "sun_combined.csv"), parse_dates=["datum"])
    sun_df["tijd"] = pd.to_datetime(sun_df["datum"], utc=True)
    
    # Fill missing values directly across your radiation options
    sun_df["radiation_wm2"] = sun_df["open_meteo_radiation"].fillna(
        sun_df["kmi_radiation_avg"]
    ).fillna(sun_df["kaggle_radiation_avg"])
    sun_df = sun_df[["tijd", "radiation_wm2"]]
    
    prod_df = pd.read_csv(os.path.join(data_dir, "productie_comnbined.csv"), parse_dates=["tijd"])
