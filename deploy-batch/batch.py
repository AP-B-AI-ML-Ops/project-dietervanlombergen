...
def get_latest_version(model_name: str) -> str:
    ...
    if not latest_versions:
        raise ValueError(f"No registered versions found for model matching name: '{model_name}'")
...
@flow()
def run_batch(input_file_path: str = "./output/val.csv", output_base_dir: str = "/batch-data/report"):
    """
...
# 2. Extract batch dataset target inputs from the unpacked pickle arrays
    ...
    df_result["prediction_error_delta"] = df_result["solar_production_mw_actual"] - df_result["solar_production_mw_predicted"]
...
#  Use the current system datetime to dynamically build the directory tree
